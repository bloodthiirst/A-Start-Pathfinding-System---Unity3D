...
public class GridController : MonoBehaviour
{
    ...
    {
        return GridAnchor - (transform.right * (GridDimensions.x / 2)) - (transform.forward * (GridDimensions.y / 2));
    }

    public Node NodeFromWorldPoint(Vector3 WordPos)
    {
        float PercentX = ((WordPos.x - GridAnchor.x) + GridDimensions.x / 2) / GridDimensions.x;
        float PercentY = ((WordPos.z - GridAnchor.z) + GridDimensions.y / 2) / GridDimensions.y;
